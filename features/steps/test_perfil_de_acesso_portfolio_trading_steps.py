...
def step_usuario_logado_trading_portfolio(context):
    context.perfil_de_acesso_portfolio_trading_pages = PerfilDeAcessoPage(
        context.driver
    )
    ...
    assert context.perfil_de_acesso_page.validar_texto_perfil(
        "Portfólio e Trading"
...
def step_verificar_acesso_modulos_produtos(context):
    ...
    context.perfil_de_acesso_page = PerfilDeAcessoPage(context.driver)
    assert (
        context.perfil_de_acesso_page.validar_modulos_produtos_existe()
...
def step_verificar_acesso_diretrizes(context):
    ...
    context.perfil_de_acesso_page = PerfilDeAcessoPage(
        context.driver
    )  # Certifique-se de inicializar o atributo correto
    assert (
        context.perfil_de_acesso_page.validar_acesso_diretrizes()
...
@then(
    'os produtos "Diário/Semanal", "I-REC" e "Curto Prazo" devem estar visíveis na tela'
)
def step_validar_produtos_visiveis(context):
    ...
    context.perfil_de_acesso_page = PerfilDeAcessoPage(context.driver)
    assert (
        context.perfil_de_acesso_page.validar_produtos_visiveis()
    ), "Um ou mais produtos não estão visíveis na tela."
    logging.info(
        "Os produtos 'Diário/Semanal', 'I-REC' e 'Curto Prazo' estão visíveis na tela."
    )
...
def step_validar_pagina_edicao_produto(context):
    ...
    context.perfil_de_acesso_page = PerfilDeAcessoPage(context.driver)
    assert (
        context.perfil_de_acesso_page.validar_pagina_edicao_produto()
...
def step_validar_tela_exclusao_produto(context):
    ...
    context.perfil_de_acesso_page = PerfilDeAcessoPage(context.driver)
    assert (
        context.perfil_de_acesso_page.validar_tela_exclusao_produto()
