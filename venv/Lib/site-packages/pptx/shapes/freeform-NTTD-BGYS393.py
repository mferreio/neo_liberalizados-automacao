...
class FreeformBuilder(Sequence[DrawingOperation]):
    ...
    def new(
        ...
        """
        return cls(
            shapes, Emu(int(round(start_x))), Emu(int(round(start_y))), x_scale, y_scale
        )

    def add_line_segments(
        self, vertices: Iterable[tuple[float, float]], close: bool = True
    ):
    ...
    def _local_to_shape(
        self, local_x: Length, local_y: Length
    ) -> tuple[Length, Length]:
