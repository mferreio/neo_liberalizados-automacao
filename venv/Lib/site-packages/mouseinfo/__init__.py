...
if sys.platform == "win32":
    ...
    def _winGetPixel(x, y):
        colorRef = ctypes.windll.gdi32.GetPixel(
            dc, x, y
        )  # A COLORREF value as 0x00bbggrr. See https://docs.microsoft.com/en-us/windows/win32/gdi/colorref
        red = colorRef % 256
        colorRef //= 256
        green = colorRef % 256
        colorRef //= 256
...
if platform.system() == "Linux":
    if RUNNING_PYTHON_2:
        try:
            ...
            sys.exit(
                "NOTE: You must install tkinter on Linux to use MouseInfo. Run the following: sudo apt-get install python-tk python-dev"
            )
    else:
        # Running Python 3+:
        try:
...
MOUSE_INFO_BUTTON_WIDTH = (
    16  # A standard width for the buttons in the MouseInfo window.
)
...
class MouseInfoWindow:
    def _updateMouseInfoTextFields(self):
        ...
        # function.
        if self.isRunning:
            self._updateMouseInfoJob = self.root.after(
                100, self._updateMouseInfoTextFields
            )
        else:
    ...
    def _logXyMouseInfo(self, *args):
        ...
        if self.delayEnabledSV.get() == "on" and len(args) == 0:
            ...
            logContents = self.logTextarea.get("1.0", "end-1c") + "%s\n" % (
                self.xyTextboxSV.get()
            )  # 'end-1c' doesn't include the final newline
    ...
    def _xyOriginChanged(self, sv):
        ...
        self.xOrigin = int(x)
        self.yOrigin = int(y)
        self.statusbarSV.set(
            "Set XY Origin to " + str(self.xOrigin) + ", " + str(self.yOrigin)
        )
    ...
    def _saveScreenshotFile(self, *args):
        ...
        try:
            ...
            self.statusbarSV.set(
                "Screenshot file saved to " + self.screenshotFilenameSV.get()
            )
    ...
    def __init__(self):
        ...
        delayCheckbox = ttk.Checkbutton(
            mainframe,
            text="3 Sec. Button Delay",
            variable=self.delayEnabledSV,
            onvalue="on",
            offvalue="off",
        )
        ...
        self.allCopyButton = ttk.Button(
            mainframe,
            textvariable=self.allCopyButtonSV,
            width=MOUSE_INFO_BUTTON_WIDTH,
            command=self._copyAllMouseInfo,
        )
        ...
        self.allLogButton = ttk.Button(
            mainframe,
            textvariable=self.allLogButtonSV,
            width=MOUSE_INFO_BUTTON_WIDTH,
            command=self._logAllMouseInfo,
        )
        ...
        self.rgbHexSV = (
            tkinter.StringVar()
        )  # The str contents of the rgb hex text field.
        self.xyOriginSV = (
            tkinter.StringVar()
        )  # The str contents of the xy origin field.
        self.logTextboxSV = (
            tkinter.StringVar()
        )  # The str contents of the log text area.
        self.logFilenameSV = (
            tkinter.StringVar()
        )  # The str contents of the log filename text field.
        self.screenshotFilenameSV = (
            tkinter.StringVar()
        )  # The str contents of the screenshot filename text field.
        self.statusbarSV = (
            tkinter.StringVar()
        )  # The str contents of the status bar at the bottom of the window.

        # WIDGETS ON ROW 3:
        CUR_ROW += 1

        # Set up the XY coordinate text field and label:
        self.xyInfoTextbox = ttk.Entry(
            mainframe, width=16, textvariable=self.xyTextboxSV
        )
        self.xyInfoTextbox.grid(column=2, row=CUR_ROW, sticky=(tkinter.W, tkinter.E))
        ttk.Label(mainframe, text="XY Position").grid(
            column=1, row=CUR_ROW, sticky=tkinter.W
        )

        # Set up the button to copy the XY coordinates to the clipboard:
        self.xyCopyButtonSV = tkinter.StringVar()
        self.xyCopyButtonSV.set("Copy XY (F2)")
        self.xyCopyButton = ttk.Button(
            mainframe,
            textvariable=self.xyCopyButtonSV,
            width=MOUSE_INFO_BUTTON_WIDTH,
            command=self._copyXyMouseInfo,
        )
        ...
        self.xyLogButton = ttk.Button(
            mainframe,
            textvariable=self.xyLogButtonSV,
            width=MOUSE_INFO_BUTTON_WIDTH,
            command=self._logXyMouseInfo,
        )
        ...
        ttk.Label(mainframe, text="RGB Color").grid(
            column=1, row=CUR_ROW, sticky=tkinter.W
        )
        ...
        self.rgbCopyButton = ttk.Button(
            mainframe,
            textvariable=self.rgbCopyButtonSV,
            width=MOUSE_INFO_BUTTON_WIDTH,
            command=self._copyRgbMouseInfo,
        )
        ...
        self.rgbLogButton = ttk.Button(
            mainframe,
            textvariable=self.rgbLogButtonSV,
            width=MOUSE_INFO_BUTTON_WIDTH,
        ...
        ttk.Label(mainframe, text="RGB as Hex").grid(
            column=1, row=CUR_ROW, sticky=tkinter.W
        )
        ...
        self.rgbHexCopyButton = ttk.Button(
            mainframe,
            textvariable=self.rgbHexCopyButtonSV,
            width=MOUSE_INFO_BUTTON_WIDTH,
        ...
        self.rgbHexLogButton = ttk.Button(
            mainframe,
            textvariable=self.rgbHexLogButtonSV,
            width=MOUSE_INFO_BUTTON_WIDTH,
            command=self._logRgbHexMouseInfo,
        )
        ...
        ttk.Label(mainframe, text="XY Origin").grid(
            column=1, row=CUR_ROW, sticky=tkinter.W
        )
        self.xyOriginSV.trace(
            "w", lambda name, index, mode, sv=self.xyOriginSV: self._xyOriginChanged(sv)
        )
        self.xyOriginSV_entry = ttk.Entry(
            mainframe, width=16, textvariable=self.xyOriginSV
        )
        ...
        self.logTextareaScrollbar = ttk.Scrollbar(
            mainframe, orient=tkinter.VERTICAL, command=self.logTextarea.yview
        )
        self.logTextareaScrollbar.grid(
            column=5, row=CUR_ROW, sticky=(tkinter.N, tkinter.S)
        )
        ...
        self.logFilenameTextbox = ttk.Entry(
            mainframe, width=16, textvariable=self.logFilenameSV
        )
        self.logFilenameTextbox.grid(
            column=1, row=CUR_ROW, columnspan=3, sticky=(tkinter.W, tkinter.E)
        )
        self.saveLogButton = ttk.Button(
            mainframe,
            text="Save Log",
            width=MOUSE_INFO_BUTTON_WIDTH,
            command=self._saveLogFile,
        )
        ...
        G_MOUSE_INFO_SCREENSHOT_FILENAME_entry = ttk.Entry(
            mainframe, width=16, textvariable=self.screenshotFilenameSV
        )
        G_MOUSE_INFO_SCREENSHOT_FILENAME_entry.grid(
            column=1, row=CUR_ROW, columnspan=3, sticky=(tkinter.W, tkinter.E)
        )
        self.saveScreenshotButton = ttk.Button(
            mainframe,
        ...
        self.screenshotFilenameSV.set(
            os.path.join(os.getcwd(), "mouseInfoScreenshot.png")
        )
        ...
        statusbar = ttk.Label(
            mainframe, relief=tkinter.SUNKEN, textvariable=self.statusbarSV
        )
        statusbar.grid(
            column=1, row=CUR_ROW, columnspan=5, sticky=(tkinter.W, tkinter.E)
        )
        ...
        self.root.option_add(
            "*tearOff", tkinter.FALSE
        )  # Disable tkinter's ugly tear-off menus which are enabled by default.

        menu = tkinter.Menu(self.root)
        self.root.config(menu=menu)

        copyMenu = tkinter.Menu(menu)
        copyMenu.add_command(
            label="Copy All",
            command=self._copyAllMouseInfo,
            accelerator="F1",
            underline=5,
        ...
        logMenu.add_command(
            label="Log XY", command=self._logXyMouseInfo, accelerator="F6", underline=4
        )
        logMenu.add_command(
            label="Log RGB",
            command=self._logRgbMouseInfo,
        ...
        helpMenu.add_command(
            ...
            command=lambda: webbrowser.open("https://mouseinfo.readthedocs.io"),
            underline=6,
        )
        ...
        self._updateMouseInfoJob = self.root.after(
            100, self._updateMouseInfoTextFields
        )  # Begin updating the text fields.
