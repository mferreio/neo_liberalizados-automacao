...
class AllureListener:
    ...
    def stop_fixture(self, parent_uuid, uuid, name, exc_type, exc_val, exc_tb):
        self.logger.stop_before_fixture(
            uuid=uuid,
            stop=now(),
    ...
    def stop_scenario(self, scenario):
        ...
        should_run = should_run and scenario.should_run_with_name_select(
            self.behave_config
        )
        should_drop_skipped_by_option = (
            scenario.status == "skipped" and not self.behave_config.show_skipped
        )
    ...
    def start_behave_step(self, step):
        ...
        if step.text:
            self.logger.attach_data(
                uuid4(), step.text, name=".text", attachment_type=AttachmentType.TEXT
            )
    ...
    def start_step(self, uuid, title, params):
        parameters = [
            Parameter(name=name, value=value) for name, value in params.items()
        ]
