...
def scenario_history_id(scenario):
    ...
    if scenario._row:
        row = scenario._row
        parts.extend(
            [f"{name}={value}" for name, value in zip(row.headings, row.cells)]
        )
    return md5(*parts)
...
def get_status_details(exc_type, exception, exc_traceback):
    if exception:
        return StatusDetails(
            message=format_exception(exc_type, exception),
...
def step_status_details(result):
    if result.exception:
        ...
        trace = (
            "\n".join(result.exc_traceback)
            if isinstance(result.exc_traceback, list)
            else format_traceback(result.exc_traceback)
        )
        return StatusDetails(
            message=format_exception(type(result.exception), result.exception),
            trace=trace,
        )
...
def is_planned_scenario(scenario, test_plan):
    if test_plan:
        ...
        for item in test_plan:
            if (allure_id and allure_id == item.get("id")) or fullname == item.get(
                "selector"
            ):
                return
