...
def description(test_description):
    return safely(
        plugin_manager.hook.decorate_as_description(test_description=test_description)
    )


def description_html(test_description_html):
    return safely(
        plugin_manager.hook.decorate_as_description_html(
            test_description_html=test_description_html
        )
    )
...
def label(label_type, *labels):
    return safely(
        plugin_manager.hook.decorate_as_label(label_type=label_type, labels=labels)
    )
...
def link(url, link_type=LinkType.LINK, name=None):
    return safely(
        plugin_manager.hook.decorate_as_link(url=url, link_type=link_type, name=name)
    )
...
class Dynamic:
    ...
    @staticmethod
    def description_html(test_description_html):
        plugin_manager.hook.add_description_html(
            test_description_html=test_description_html
        )
    ...
    def parameter(name, value, excluded=None, mode: Union[ParameterMode, None] = None):
        plugin_manager.hook.add_parameter(
            name=name, value=value, excluded=excluded, mode=mode
        )
...
class Attach:
    ...
    def __call__(self, body, name=None, attachment_type=None, extension=None):
        plugin_manager.hook.attach_data(
            body=body, name=name, attachment_type=attachment_type, extension=extension
        )

    def file(self, source, name=None, attachment_type=None, extension=None):
        plugin_manager.hook.attach_file(
            source=source,
            name=name,
...
class fixture:
    ...
    def __enter__(self):
        plugin_manager.hook.start_fixture(
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        plugin_manager.hook.stop_fixture(
            parent_uuid=self._parent_uuid,
            uuid=self._uuid,
            name=self._name,
            exc_type=exc_type,
...
class test:
    ...
    def __enter__(self):
        plugin_manager.hook.start_test(
            parent_uuid=None,
            uuid=self._uuid,
            name=None,
            parameters=self.parameters,
