...
def _locateAll_pillow(
    ...
    if (
        grayscale
    ):  # if grayscale mode is on, convert the needle and haystack images to grayscale
    ...
    needleImageRows = [
        needleImageData[y * needleWidth : (y + 1) * needleWidth]
        for y in range(needleHeight)
    ]  # LEFT OFF - check this
...
def locateOnWindow(image, title, **kwargs):
    ...
    return locateOnScreen(
        image, region=(win.left, win.top, win.width, win.height), **kwargs
    )
...
def _screenshot_linux(imageFilename=None, region=None):
    ...
    if PILLOW_VERSION >= (9, 2, 0) and GNOMESCREENSHOT_EXISTS:
        ...
        if region is None:
            ...
            assert (
                len(region) == 4
            ), "region argument must be a tuple of four ints"  # TODO fix this
            assert (
                isinstance(region[0], int)
                and isinstance(region[1], int)
                and isinstance(region[2], int)
