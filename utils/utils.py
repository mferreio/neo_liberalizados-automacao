...
def gerar_documento_evidencia(nome_teste, sucesso=True, erros=None):
    ...
    try:
        ...
        modelo = (
            "modelos de evidencias/1.evidencia.docx"
            if sucesso
            else "modelos de evidencias/2.bug.docx"
        )
        nome_arquivo = (
            f"reports/evidencias/{'Evidência' if sucesso else 'Bug'}_{data_teste}.docx"
        )
...
def gerar_resumo_testes(total_testes, testes_sucesso, testes_falha):
    ...
    data_teste = datetime.now().strftime(
        "%Y-%m-%d_%H-%M-%S"
    )  # Corrigido para usar datetime.now()
